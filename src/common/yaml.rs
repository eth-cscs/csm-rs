...
//! [`Error::YamlShape`] with field context so missing or wrong-typed
//! fields surface as actionable, structured errors.
...
pub(crate) fn yaml_str<'a>(
  ...
  value
    ...
    .ok_or_else(|| {
      Error::YamlShape(format!("missing or non-string field '{key}'"))
    })
}

/// Get `value[key]` as `&Vec<serde_yaml::Value>` (a sequence), or
/// [`Error::YamlShape`] with field context.
pub(crate) fn yaml_seq<'a>(
  ...
  value
    ...
    .ok_or_else(|| {
      Error::YamlShape(format!("missing or non-sequence field '{key}'"))
    })
}

/// Convert a `serde_yaml::Value` to `&str`, or [`Error::YamlShape`].
/// Used when a known-existing `Value` is expected to be a string (e.g.
/// inside a chain where the caller already extracted the value but
/// wants to fail cleanly if it's the wrong type).
pub(crate) fn as_yaml_str(value: &serde_yaml::Value) -> Result<&str, Error> {
  value.as_str().ok_or_else(|| {
    Error::YamlShape("value is not a string".to_string())
  })
}
